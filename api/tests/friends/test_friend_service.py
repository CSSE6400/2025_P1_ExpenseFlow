...
@pytest.mark.asyncio
async def test_add_friend(session: AsyncSession, user_model_factory: UserModelFactory):
    sender = user_model_factory.build()
    receiver = user_model_factory.build()
    friend_model = await create_accept_friend_request(session, sender, receiver)
    assert friend_model
    ...
    accepted_friend_model = await create_accept_friend_request(
...
async def test_accepted_request(
    session: AsyncSession, user_model_factory: UserModelFactory
):
    sender = user_model_factory.build()
    receiver = user_model_factory.build()
...
async def test_fuzz_create_friend(
    ...
    assert str(eferror.value) == f"User '{sender.user_id}' cannot friend itself."

    with pytest.raises(ExpenseFlowError) as eferror:
        await create_accept_friend_request(session, receiver, receiver)
    assert str(eferror.value) == f"User '{receiver.user_id}' cannot friend itself."
